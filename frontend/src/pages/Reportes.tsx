...
export default function Reportes() {
  ...
  return (
    <div className="module-page reportes-page">
      <header className="module-page-header"><div><span className="module-eyebrow">Inteligencia operativa</span><h1 className="module-title flex items-center gap-3"><BarChart3 className="text-amber-700" /> Reportes</h1><p className="module-description">Indicadores verificables de operación, conversión y finanzas.</p></div><button onClick={load} disabled={loading} className="btn btn-secondary btn-md"><RefreshCw size={17} className={loading ? 'animate-spin' : ''} /> Actualizar</button></header>

      <section className="surface-card p-5 space-y-4"><div className="flex items-center gap-2 text-[13px] font-bold uppercase tracking-wide text-slate-600"><Filter size={16} /> Filtros del reporte</div><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-6">
        ...
        {filters.periodo === 'PERSONALIZADO' && <div className="flex gap-2 sm:col-span-2"><input type="date" aria-label="Fecha inicial" value={filters.desde} onChange={(e) => update('desde', e.target.value)} className="control-height min-w-0 w-1/2 rounded-xl border border-slate-300 px-3 text-sm" /><input type="date" aria-label="Fecha final" value={filters.hasta} onChange={(e) => update('hasta', e.target.value)} className="control-height min-w-0 w-1/2 rounded-xl border border-slate-300 px-3 text-sm" /></div>}
      </div></section>

      {error && <div className="p-4 rounded-xl border border-rose-200 bg-rose-50 text-sm text-rose-800">{error}</div>}
      <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">{cards.map((card) => <article key={card.label} className="metric-card"><div className="flex items-center justify-between"><p className="metric-label normal-case tracking-normal">{card.label}</p><card.icon size={21} className="text-amber-700" /></div><p className="metric-value truncate" title={String(card.value)}>{card.value}</p><p className="mt-2 text-[13px] text-slate-500">{card.note}</p></article>)}</section>

      {loading ? <div className="p-16 text-center text-slate-500">Calculando indicadores…</div> : (
        <div className="grid gap-5 xl:grid-cols-2">
          <ChartCard title="Expedientes por tipo de acto" subtitle="Distribución de nuevas operaciones"><ResponsiveContainer width="100%" height={290}><BarChart data={data?.desglose?.por_tipo_acto || []} layout="vertical" margin={{ left: 16 }}><CartesianGrid strokeDasharray="3 3" horizontal={false} /><XAxis type="number" allowDecimals={false} /><YAxis type="category" dataKey="nombre" width={125} tick={{ fontSize: 12 }} /><Tooltip /><Bar dataKey="total" name="Expedientes" fill="#d97706" radius={[0, 6, 6, 0]} /></BarChart></ResponsiveContainer></ChartCard>
          <ChartCard title="Distribución por estado" subtitle="Carga operativa del periodo"><ResponsiveContainer width="100%" height={290}><PieChart><Pie data={data?.desglose?.por_estatus || []} dataKey="total" nameKey="nombre" innerRadius={62} outerRadius={102} paddingAngle={2}>{(data?.desglose?.por_estatus || []).map((_: any, index: number) => <Cell key={index} fill={COLORS[index % COLORS.length]} />)}</Pie><Tooltip /><Legend wrapperStyle={{ fontSize: 12 }} /></PieChart></ResponsiveContainer></ChartCard>
          <ChartCard title="Carga por abogado" subtitle="Expedientes abiertos en el periodo"><ResponsiveContainer width="100%" height={290}><BarChart data={data?.desglose?.por_abogado || []}><CartesianGrid strokeDasharray="3 3" vertical={false} /><XAxis dataKey="nombre" tick={{ fontSize: 12 }} /><YAxis allowDecimals={false} /><Tooltip /><Bar dataKey="total" name="Expedientes" fill="#1d4ed8" radius={[6, 6, 0, 0]} /></BarChart></ResponsiveContainer></ChartCard>
          <ChartCard title="Tendencia de operación" subtitle="Aperturas, firmas y entregas por mes"><ResponsiveContainer width="100%" height={290}><LineChart data={data?.desglose?.tendencia || []}><CartesianGrid strokeDasharray="3 3" /><XAxis dataKey="periodo" tick={{ fontSize: 12 }} /><YAxis allowDecimals={false} /><Tooltip /><Legend /><Line type="monotone" dataKey="nuevos" stroke="#1d4ed8" strokeWidth={2} /><Line type="monotone" dataKey="firmados" stroke="#d97706" strokeWidth={2} /><Line type="monotone" dataKey="entregados" stroke="#0f766e" strokeWidth={2} /></LineChart></ResponsiveContainer></ChartCard>
...
function ChartCard({ title, subtitle, children }: { title: string; subtitle: string; children: React.ReactNode }) {
  return <section className="min-w-0 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><h2 className="font-bold text-slate-950">{title}</h2><p className="mb-5 mt-1 text-[13px] text-slate-500">{subtitle}</p><div className="min-w-0 overflow-hidden">{children}</div></section>;
}
