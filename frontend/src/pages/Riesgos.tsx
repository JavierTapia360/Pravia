...
export default function Riesgos() {
  ...
  return (
    <div className="mx-auto max-w-[1500px] space-y-6">
      <section className="rounded-2xl border border-slate-200 bg-slate-950 px-6 py-7 text-white shadow-sm lg:px-8">
        <div className="flex flex-col justify-between gap-5 lg:flex-row lg:items-end"><div><p className="flex items-center gap-2 text-sm font-semibold text-amber-300"><ShieldAlert size={18} />Cumplimiento explicable y versionado</p><h1 className="mt-2 text-3xl font-semibold">Riesgos, UIF e ISR</h1><p className="mt-3 max-w-3xl text-sm leading-6 text-slate-300">Cuestionarios, evidencia y resultados revisables. Ninguna conclusión sustituye la responsabilidad jurídica o fiscal.</p></div><button type="button" onClick={openNew} className="inline-flex items-center justify-center gap-2 rounded-lg bg-amber-400 px-4 py-2.5 text-sm font-semibold text-slate-950 hover:bg-amber-300"><Plus size={17} />Nueva revisión</button></div>
      </section>

      <div className="flex gap-2 rounded-xl border border-slate-200 bg-white p-2 shadow-sm"><button type="button" onClick={() => { setTab('UIF'); setSelected(null); }} className={`flex flex-1 items-center justify-center gap-2 rounded-lg px-4 py-2.5 text-sm font-semibold ${tab === 'UIF' ? 'bg-slate-950 text-white' : 'text-slate-600 hover:bg-slate-50'}`}><Landmark size={17} />UIF</button><button type="button" onClick={() => { setTab('ISR'); setSelected(null); }} className={`flex flex-1 items-center justify-center gap-2 rounded-lg px-4 py-2.5 text-sm font-semibold ${tab === 'ISR' ? 'bg-slate-950 text-white' : 'text-slate-600 hover:bg-slate-50'}`}><Calculator size={17} />ISR</button></div>

      <section className="grid gap-4 sm:grid-cols-3"><article className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm"><p className="text-sm text-slate-500">Revisiones</p><p className="mt-2 text-2xl font-semibold">{counts.total}</p></article><article className="rounded-xl border border-amber-200 bg-amber-50 p-5"><p className="text-sm text-amber-700">Pendientes de revisión</p><p className="mt-2 text-2xl font-semibold text-amber-950">{counts.pending}</p></article><article className="rounded-xl border border-emerald-200 bg-emerald-50 p-5"><p className="text-sm text-emerald-700">Confirmadas</p><p className="mt-2 text-2xl font-semibold text-emerald-950">{counts.confirmed}</p></article></section>

      <section className="grid gap-5 xl:grid-cols-[0.82fr_1.18fr]">
        <div className="space-y-5">
          <article className="rounded-xl border border-slate-200 bg-white shadow-sm"><div className="flex items-center justify-between border-b border-slate-200 px-5 py-4"><div><h2 className="font-semibold text-slate-950">Revisiones {tab}</h2><p className="text-xs text-slate-500">Casos con versión normativa fija</p></div><button onClick={() => void load()} className="rounded-lg p-2 text-slate-500 hover:bg-slate-100"><RefreshCw className={loading ? 'animate-spin' : ''} size={16} /></button></div>{reviews.length ? <div className="divide-y divide-slate-100">{reviews.map((review) => <button type="button" key={review.id} onClick={() => setSelected(review)} className={`flex w-full items-center gap-3 px-5 py-4 text-left hover:bg-slate-50 ${selected?.id === review.id ? 'bg-indigo-50' : ''}`}><span className={`h-2.5 w-2.5 rounded-full ${review.estatus === 'CONFIRMADO' ? 'bg-emerald-500' : review.estatus === 'PENDIENTE_REVISION' ? 'bg-amber-500' : 'bg-slate-300'}`} /><span className="min-w-0 flex-1"><strong className="block text-sm text-slate-900">{review.expediente.numero_pravia}</strong><small className="block truncate text-slate-500">{review.expediente.cliente_alias || review.ruleSet.nombre}</small><small className="text-[11px] text-slate-400">{labelStatus(review.estatus)} · {review.rule_version_snapshot}</small></span><ChevronRight size={17} className="text-slate-400" /></button>)}</div> : <div className="px-6 py-12 text-center text-sm text-slate-500"><ClipboardCheck className="mx-auto mb-3 text-slate-300" size={32} />No hay revisiones {tab}.</div>}</article>
