...
import { executeAssistantTool } from './assistantTools.service';

const user = (permissions: string[] = ['expedientes.read']) => ({
...
describe('assistant backend tools', () => {
  it('deniega la tool antes de consultar datos si falta el permiso', async () => {
    const client = db();
    await expect(executeAssistantTool({ tool: 'getFinancialSummary', args: { expediente_id: 'exp-1' }, user: user([]), correlationId: 'corr-1' }, client))
  ...
  it('responde estructurado, con procedencia y audita una lectura permitida', async () => {
    ...
    expect(client.auditLog.create).toHaveBeenCalledWith(expect.objectContaining({ data: expect.objectContaining({ accion: 'AI_TOOL_READ', correlation_id: 'corr-4' }) }));
  });
  ...
  it('prepara una acción sin persistirla ni ejecutar la API normal', async () => {
    ...
    const result = await executeAssistantTool({ tool: 'prepareTask', args: { title: 'Revisar documentos', fecha_limite: '2026-08-14', expediente_id: 'exp-1' }, user: user(['agenda.write']), correlationId: 'corr-5' }, client);
